...
serve(async (req: Request) => {
  ...
  try {
    const { blocked_owner_id, duration_minutes, reason } = await req.json();

    if (!blocked_owner_id || !duration_minutes || !reason) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: blocked_owner_id, duration_minutes, reason' }),
    ...
    if (adminRole?.role !== 'administrator' && adminRole?.role !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Only administrators can block owners' }),
        { status: 403, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // Проверить что владелец уже заблокирован
    const { data: existingBlock } = await supabase
      .from('owner_blocks')
      .select('id')
      .eq('blocked_owner_id', blocked_owner_id)
      .eq('status', 'active')
      .maybeSingle();

    if (existingBlock) {
      return new Response(
        JSON.stringify({ error: 'Owner is already blocked' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
