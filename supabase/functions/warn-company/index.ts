...
serve(async (req: Request) => {
  ...
  try {
    const { warned_company_id, reason, severity = 'mild' } = await req.json();

    if (!warned_company_id || !reason) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: warned_company_id, reason' }),
    ...
    if (adminProfile.role !== 'administrator' && adminProfile.role !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Only administrators can warn companies' }),
        { status: 403, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // Получить количество предыдущих предупреждений компании
    const { data: prevWarnings } = await supabase
      .from('company_warnings')
      .select('id')
      .eq('warned_company_id', warned_company_id);
    ...
    if (warningCount >= 2) {
      ...
      const { data: block } = await supabase
        .from('company_blocks')
        .insert({
          blocked_company_id: warned_company_id,
    ...
    // Создать предупреждение компании
