...
serve(async (req: Request) => {
  ...
  try {
    const { warned_user_id, reason, severity = 'mild' } = await req.json();

    if (!warned_user_id || !reason) {
      return new Response(
        JSON.stringify({ error: 'Missing required fields: warned_user_id, reason' }),
    ...
    if (adminProfile.role !== 'administrator' && adminProfile.role !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Only administrators can warn users' }),
        { status: 403, headers: { 'Content-Type': 'application/json' } }
      );
    }

    // Получить количество предыдущих предупреждений
    const { data: prevWarnings } = await supabase
      .from('user_warnings')
      .select('id')
      .eq('warned_user_id', warned_user_id);
    ...
    if (warningCount >= 2) {
      ...
      const { data: block } = await supabase
        .from('user_blocks')
        .insert({
          blocked_user_id: warned_user_id,
